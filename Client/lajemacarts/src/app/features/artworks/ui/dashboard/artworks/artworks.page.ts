...
export class ArtworksPage {
  ...
  protected readonly currentFilters = computed((): ArtworkFilters => {
    return this.store.getFiltersForComponent();
  });
  ...
  private observeRequestStatus(action: string): void {
    toObservable(this.store.requestStatus, {injector: this.injector}).pipe(
      ...
      if (finalStatus === 'fulfilled') {
        this.snackBar.open(`${action} réussie !`, 'OK', {
          duration: 3000,
          panelClass: ['success-snackbar']
        });
      } else {
        this.snackBar.open(`Échec : ${this.store.error()}`, 'Fermer', {
          duration: 5000,
